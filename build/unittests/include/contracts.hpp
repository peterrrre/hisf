...
#define MAKE_READ_WASM_ABI(CN,C, D) \
   static std::vector<uint8_t> CN ## _wasm() { return read_wasm("/home/a/test/dimension/build/unittests/" #D "/" #C "/" #C ".wasm"); } \
   static std::vector<char> CN ## _abi() { return read_abi("/home/a/test/dimension/build/unittests/" #D "/" #C "/" #C ".abi"); }

namespace eosio {
   namespace testing {
      struct contracts {
         // Contracts in `eos/unittests/contracts' directory
         MAKE_READ_WASM_ABI(eosio_bios,   eonio.bios, contracts)
         MAKE_READ_WASM_ABI(eosio_msig,   eonio.msig, contracts)
         MAKE_READ_WASM_ABI(eosio_system, eonio.system, contracts)
         MAKE_READ_WASM_ABI(eosio_token,  eonio.token,  contracts)
         MAKE_READ_WASM_ABI(eosio_wrap,   eonio.wrap,   contracts)
